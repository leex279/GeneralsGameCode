...
def test_capture_flags_are_closed_and_modern_zero_hour_only(repository_root: Path) -> None:
    ...
    command_line = _source(
        repository_root, "GeneralsMD/Code/GameEngine/Source/Common/CommandLine.cpp"
    )
    ...
    capture_section = command_line.split(
        "static void validateReplayCompatRunnerOptions()", maxsplit=1
    )[1].split("#endif", maxsplit=1)[0]
    assert all(flag in capture_section for flag in ("-recordVideo", "-videoRes", "-videoFps"))
    assert "strtol" in command_line
    assert "fps != 30 && fps != 60" in command_line
    ...
    assert "GetFileAttributesA" in capture_section
    assert 'concat(".capture-result.json")' in capture_section
    assert "capture output already exists" in capture_section
    assert "capture result sidecar already exists" in capture_section
    assert "#if defined(RTS_REPLAY_COMPAT_RUNNER) && !defined(IS_VS6_BUILD)" in command_line
    assert "m_recordVideoPath" in zero_hour_header
    assert "m_replayCompatRunnerFailed" in zero_hour_header
...
def test_partial_startup_preflight_uses_immediate_os_termination(repository_root: Path) -> None:
    """A rejected stale output must not enter unsafe CRT teardown before engine init."""
    command_line = _source(
        repository_root, "GeneralsMD/Code/GameEngine/Source/Common/CommandLine.cpp"
    )
    error_handler = command_line.split(
        "static void replayCompatRunnerCommandLineError(const char *message)", maxsplit=1
    )[1].split("static Bool isAbsoluteReplayCompatRunnerPath", maxsplit=1)[0]

    assert "fflush(stderr)" in error_handler
    assert "ExitProcess(1)" in error_handler
    assert "exit(1)" not in error_handler


def test_presentation_preflight_rejects_replay_wildcards(repository_root: Path) -> None:
    """One raw replay argument must not expand into multiple presentation jobs later."""
    command_line = _source(
        repository_root, "GeneralsMD/Code/GameEngine/Source/Common/CommandLine.cpp"
    )
    validation = command_line.split(
        "static void validateReplayCompatRunnerOptions()", maxsplit=1
    )[1].split("#endif", maxsplit=1)[0]

    assert "strpbrk" in validation
    assert '"*?"' in validation
    assert "presentation replay path must not contain wildcards" in validation


def test_rendered_runner_propagates_replay_start_failure(repository_root: Path) -> None:
    """An unreadable camera-only replay must return nonzero without changing baseline builds."""
    replay_simulation = _source(
        repository_root, "Core/GameEngine/Source/Common/ReplaySimulation.cpp"
    )
    rendered_branch = replay_simulation.split("if (!TheGlobalData->m_headless)", maxsplit=1)[
        1
    ].split("// Note that we use printf here", maxsplit=1)[0]

    assert "const Bool playbackStarted = TheRecorder->playbackFile" in rendered_branch
    assert "#if defined(RTS_REPLAY_COMPAT_RUNNER) && !defined(IS_VS6_BUILD)" in rendered_branch
    failure_branch = rendered_branch.split("if (!playbackStarted)", maxsplit=1)[1].split(
        "#endif", maxsplit=1
    )[0]
    assert "numErrors++" in failure_branch
    assert "continue" in failure_branch


def test_presentation_runner_pins_client_cadence_to_sixty_hz(repository_root: Path) -> None:
    """Client animation and camera updates must not depend on user or INI FPS settings."""
    game_engine = _source(
        repository_root, "GeneralsMD/Code/GameEngine/Source/Common/GameEngine.cpp"
    )
    init_cadence = game_engine.split(
        "CommandLine::parseCommandLineForEngineInit();", maxsplit=1
    )[1].split("TheArchiveFileSystem->loadMods();", maxsplit=1)[0]
    assert "m_recordVideoPath.isEmpty()" in init_cadence
    assert "m_autoCameraScriptPath.isEmpty()" in init_cadence
    assert "m_framesPerSecondLimit = 60" in init_cadence
    assert "m_useFpsLimit = TRUE" in init_cadence
...
int main()
{
	...
	if (W3DVideoCaptureContract::logicFrameStrideForFps(30) != 2) return 8;
	if (W3DVideoCaptureContract::logicFrameStrideForFps(60) != 1) return 9;
	if (W3DVideoCaptureContract::logicFrameStrideForFps(59) != 0) return 10;
	if (!W3DVideoCaptureContract::shouldPresentLogicFrame(30, 0)) return 15;
	if (W3DVideoCaptureContract::shouldPresentLogicFrame(30, 1)) return 16;
	if (!W3DVideoCaptureContract::shouldPresentLogicFrame(30, 2)) return 17;
	if (!W3DVideoCaptureContract::shouldPresentLogicFrame(60, 1)) return 18;
...
def test_capture_failures_and_sidecar_are_typed(repository_root: Path) -> None:
    ...
    assert "if (m_failure == W3D_VIDEO_CAPTURE_OK && (!m_opened || m_presentationFrames == 0))" in close_section
    ...
    assert "width != static_cast<unsigned int>(m_requestedWidth)" in open_section
    assert "height != static_cast<unsigned int>(m_requestedHeight)" in open_section
    ...
    assert "++m_logicFrames" in capture_section
    assert "shouldPresentLogicFrame(m_fps, logicFrame)" in capture_section
    assert "++m_presentationFrames" in capture_section
...
def test_display_captures_once_per_logic_frame_and_replay_owns_shutdown(
    ...
    assert "if (!s_replayVideoWriter->captureFrame" in capture_hook
    assert "m_replayCompatRunnerFailed = TRUE" in capture_hook
    assert "TheGameEngine->setQuitting(TRUE)" in capture_hook
    assert "exit(1);" not in capture_hook
    assert "close" not in capture_hook
    assert "m_lastLogicFrame" in writer
    assert "logicFrameStrideForFps" in writer
    assert "shouldPresentLogicFrame" in writer
    assert "setQuitting" not in writer
    assert "update()" not in writer
    assert "TiVO" not in writer

    close_hook = display.split("void W3DDisplay::draw()", maxsplit=1)[1].split(
        "updateAverageFPS();", maxsplit=1
    )[0]
    assert "TheRecorder->sawCRCMismatch()" in close_hook
    assert "!TheRecorder->isPlaybackInProgress()" in close_hook
    assert "finalizeReplayVideoCapture()" in close_hook
    assert "s_replayVideoWriter->hasFailed()" in close_hook
    assert "m_replayCompatRunnerFailed = TRUE" in close_hook

    game_main = _source(repository_root, "GeneralsMD/Code/GameEngine/Source/Common/GameMain.cpp")
    assert "TheGlobalData->m_replayCompatRunnerFailed" in game_main
    assert "exitcode = 1" in game_main


def test_game_main_finalizes_capture_before_sampling_runner_status(
    repository_root: Path,
) -> None:
    """Catch FFmpeg close failures becoming visible only during display destruction."""
    display_header = _source(repository_root, "Core/GameEngine/Include/GameClient/Display.h")
    w3d_header = _source(
        repository_root,
    ...
    assert "virtual void finalizeReplayVideoCapture()" in display_header
    assert "void finalizeReplayVideoCapture() override" in w3d_header
    finalizer = w3d_source.split(
        "void W3DDisplay::finalizeReplayVideoCapture()", maxsplit=1
    )[1].split("void W3DDisplay::draw()", maxsplit=1)[0]
    assert "s_replayVideoWriter->close()" in finalizer
    assert "s_replayVideoWriter->hasFailed()" in finalizer
    assert "m_replayCompatRunnerFailed = TRUE" in finalizer

    finalization_call = game_main.index("TheDisplay->finalizeReplayVideoCapture()")
    status_sample = game_main.index("TheGlobalData->m_replayCompatRunnerFailed")
    engine_delete = game_main.index("delete TheGameEngine")
    assert finalization_call < status_sample < engine_delete
    assert "W3DDisplay.h" not in game_main
...
def test_top_level_build_wiring_preserves_protected_device_cmake(repository_root: Path) -> None:
    ...
    device_cmake = _source(repository_root, "Core/GameEngineDevice/CMakeLists.txt")
    zero_hour_device_cmake = _source(repository_root, "GeneralsMD/Code/GameEngineDevice/CMakeLists.txt")
    base_game_cmake = _source(repository_root, "Generals/Code/GameEngine/CMakeLists.txt")
    assert "W3DVideoWriter.cpp" not in device_cmake
    assert "W3DVideoWriter.h" not in device_cmake
    assert "if(RTS_REPLAY_COMPAT_RUNNER AND NOT IS_VS6_BUILD)" in zero_hour_device_cmake
    assert "list(APPEND GAMEENGINEDEVICE_SRC" in zero_hour_device_cmake
    assert "${CMAKE_SOURCE_DIR}/Core/GameEngineDevice/Include/W3DDevice/GameClient/W3DVideoWriter.h" in zero_hour_device_cmake
    assert "${CMAKE_SOURCE_DIR}/Core/GameEngineDevice/Source/W3DDevice/GameClient/W3DVideoWriter.cpp" in zero_hour_device_cmake
    assert "W3DVideoWriter" not in base_game_cmake
...
def test_zero_hour_globaldata_stacktrace_field_has_target_independent_layout(
    ...
    runner_definition = "add_compile_definitions(RTS_REPLAY_COMPAT_RUNNER=1)"

    assert "target_compile_definitions(z_gameenginedevice PRIVATE" not in root_cmake
    assert runner_definition in root_cmake
    assert "RTS_REPLAY_COMPAT_RUNNER AND NOT CMAKE_SIZEOF_VOID_P EQUAL 4" in root_cmake
    assert "requires a 32-bit build" in root_cmake
    assert "#if defined(RTS_REPLAY_COMPAT_RUNNER) && !defined(IS_VS6_BUILD)" in global_data_header
    assert "#if defined(RTS_REPLAY_COMPAT_RUNNER) && !defined(IS_VS6_BUILD)" in global_data_source
...
def test_rendered_analyzer_replay_exits_when_playback_reaches_terminal_state(repository_root: Path) -> None:
    ...
    assert "TheGlobalData->m_autoCameraScriptPath.isEmpty()" in source
...
def test_rendered_capture_records_crc_mismatch_without_focus_dependent_pause(repository_root: Path) -> None:
    ...
    assert "m_autoCameraScriptPath.isEmpty()" in mismatch
