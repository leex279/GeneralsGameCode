...
"""
    ...
    # TheSuperHackers @fix Leex 24/08/2026 Match installed-resource proofs to the loader's cross-platform LF identity. (#TBD)
    proof_environment["TEST_PROMPT_SHA256"] = (
        __import__("hashlib")
        .sha256(
            (project / "src" / "generals_replay_analyzer" / "data" / resource_names[0])
            .read_bytes()
            .replace(b"\r\n", b"\n")
        )
        .hexdigest()
    ...
    proof_environment["TEST_SCHEMA_SHA256"] = (
        __import__("hashlib")
        .sha256(
            (project / "src" / "generals_replay_analyzer" / "data" / resource_names[1])
            .read_bytes()
            .replace(b"\r\n", b"\n")
        )
        .hexdigest()
