...
void RecorderClass::CRCInfo::addCRC(UnsignedInt val, UnsignedInt frame)
{
...
#if defined(RTS_REPLAY_ANALYZER)
	// TheSuperHackers @feature Leex 20/08/2026 Discard reset transactions because reset is not an authoritative replay termination. (#TBD)
...
void RecorderClass::stopPlayback() {
	if (m_file != nullptr) {
#if defined(RTS_REPLAY_ANALYZER)
		// TheSuperHackers @feature Leex 20/08/2026 Finish telemetry with the replay source's explicit authoritative termination reason. (#TBD)
		ReplayTelemetryTerminationReason telemetryTermination = REPLAY_TELEMETRY_TERMINATION_INTERRUPTED;
		if (sawCRCMismatch())
		{
			telemetryTermination = REPLAY_TELEMETRY_TERMINATION_CRC_MISMATCH;
		}
		else if (!m_replayParseDumpComplete)
		{
			telemetryTermination = REPLAY_TELEMETRY_TERMINATION_TRUNCATED_INPUT;
		}
		else if (m_nextFrame == (UnsignedInt)-1)
		{
			telemetryTermination = REPLAY_TELEMETRY_TERMINATION_CLEAN_EOF;
		}
		if (telemetryTermination == REPLAY_TELEMETRY_TERMINATION_CLEAN_EOF)
		{
			// TheSuperHackers @feature Leex 18/08/2026 Defer clean completion until terminal-frame GameLogic work has executed. (#TBD)
			ReplayTelemetry::deferFinish(telemetryTermination);
		}
		else
		{
			ReplayTelemetry::finish(TheGameLogic != nullptr ? TheGameLogic->getFrame() : 0, telemetryTermination);
		}
...
Bool RecorderClass::readReplayHeader(ReplayHeader& header)
{
	...
	// TheSuperHackers @build Leex 23/08/2026 Reuse one loop index because MSVC 6 keeps for-init variables in function scope. (#TBD)
	Int playerDisconnectIndex = 0;
	for (playerDisconnectIndex=0; playerDisconnectIndex<MAX_SLOTS; ++playerDisconnectIndex)
	{
		fixedHeaderComplete = m_file->read(&(playerDiscons[playerDisconnectIndex]), sizeof(Bool)) == sizeof(Bool)
			&& fixedHeaderComplete;
	}
	...
	for (playerDisconnectIndex=0; playerDisconnectIndex<MAX_SLOTS; ++playerDisconnectIndex)
	{
		header.playerDiscons[playerDisconnectIndex] = playerDiscons[playerDisconnectIndex];
	}
...
void RecorderClass::handleCRCMessage(UnsignedInt newCRC, Int playerIndex, Bool fromPlayback, Bool hasSnapshotFrame, UnsignedInt snapshotFrame)
{
	...
	if (isLocalPlayer)
	{
		...
		if (TheGameLogic->getFrame() > 0 && newCRC != playbackCRC.value && !m_crcInfo.sawCRCMismatch())
		{
			...
			// TheSuperHackers @fix Leex 23/08/2026 Report the exact queued CRC snapshot frame instead of a receive-frame queue offset. (#TBD)
			const UnsignedInt mismatchFrame = playbackCRC.frame;
#if defined(RTS_REPLAY_ANALYZER) && !defined(IS_VS6_BUILD)
			...
			TheInGameUI->message(mismatchDetailsStr, playbackCRC.value, newCRC, mismatchFrame);

			DEBUG_LOG(("Replay has gone out of sync!\nInGame:%8.8X Replay:%8.8X\nFrame:%d",
				playbackCRC.value, newCRC, mismatchFrame));
			...
			fflush(stdout);
#if defined(RTS_REPLAY_ANALYZER) && !defined(IS_VS6_BUILD)
			// TheSuperHackers @feature Leex 24/08/2026 Mark capture-owned CRC termination independently of focus so native video cannot stall on a paused replay. (#TBD)
...
Bool RecorderClass::playbackFile(AsciiString filename)
{
	...
	// TheSuperHackers @bugfix Leex 23/08/2026 Align replay CRC warmup with recorded human multiplayer slots instead of the local IP sentinel. (#TBD)
	Bool isMultiplayer = m_gameInfo.isMultiPlayer();
