	...
	m_sidecarWritten(false)
{
	if (m_outputPath.empty() || requestedWidth <= 0 || requestedHeight <= 0
		|| W3DVideoCaptureContract::logicFrameStrideForFps(fps) == 0)
	{
...
bool W3DVideoWriter::open(IDirect3DDevice8 *device, unsigned int width, unsigned int height, DWORD format)
{
	...
	// TheSuperHackers @bugfix Leex 24/08/2026 Reject a mismatched backbuffer instead of stretching an unintended render mode. (#TBD)
	if (width != static_cast<unsigned int>(m_requestedWidth)
		|| height != static_cast<unsigned int>(m_requestedHeight))
	{
		fail(W3D_VIDEO_CAPTURE_INVALID_CONFIGURATION, ERROR_INVALID_DATA);
		return false;
	}
...
bool W3DVideoWriter::captureFrame(IDirect3DDevice8 *device, unsigned int logicFrame)
{
	...
	++m_logicFrames;
	if (!W3DVideoCaptureContract::shouldPresentLogicFrame(m_fps, logicFrame))
	{
		return true;
	}
...
{
	if (m_closed) return;
	// TheSuperHackers @fix Leex 24/08/2026 Preserve the first typed capture failure during finalization. (#TBD)
	if (m_failure == W3D_VIDEO_CAPTURE_OK && (!m_opened || m_presentationFrames == 0))
	{
		m_failure = W3D_VIDEO_CAPTURE_OUTPUT_MISSING;
