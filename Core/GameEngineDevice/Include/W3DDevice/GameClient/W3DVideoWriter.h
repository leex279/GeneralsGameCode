...
namespace W3DVideoCaptureContract
{
	...
	// TheSuperHackers @bugfix Leex 24/08/2026 Sample the Generals Online 60 Hz replay clock without changing simulation cadence. (#TBD)
	inline int logicFrameStrideForFps(int fps)
	{
		return fps == 30 ? 2 : (fps == 60 ? 1 : 0);
	}

	// TheSuperHackers @bugfix Leex 24/08/2026 Anchor sampling to absolute logic-frame parity so repeated captures select identical frames. (#TBD)
	inline bool shouldPresentLogicFrame(int fps, unsigned int logicFrame)
	{
		const int stride = logicFrameStrideForFps(fps);
		return stride > 0 && logicFrame % static_cast<unsigned int>(stride) == 0;
	}
}
