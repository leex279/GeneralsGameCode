...
Set `GENERALS_REPLAY_ANALYZER_ENGINE_EXECUTABLE` to the absolute instrumented `generalszh.exe` build path and `GENERALS_REPLAY_ANALYZER_ENGINE_RUNTIME_DIRECTORY` to the legal Zero Hour installation directory, then restart both worker and web processes. The runtime directory supplies the game's DLLs and data. Only when the configured build and runtime directories differ does the analyzer create a temporary run-unique hardlink beside those modules; on Windows an exact held handle prevents writes, rename, replacement, or deletion until the child tree settles, then removes only that owned link by handle. The retail executable is never replaced. Telemetry is unavailable without the instrumented build, but replay-byte inspection remains available.
